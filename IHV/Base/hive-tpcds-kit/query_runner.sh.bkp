...
function generateZipReport() {
    ...
    FINAL_REPORT_LOCATION="<HDFS LOCATION HERE>/${SCALE}/"
...
-bash-4.2$
-bash-4.2$ nohup sh query_runner.sh 500 orc  > 0_Logs/Feb22_100 &
[1] 14244
-bash-4.2$ nohup: ignoring input and redirecting stderr to stdout

-bash-4.2$
-bash-4.2$
-bash-4.2$ tail -f 0_Logs/Feb22_100
Old clock removed
Created new clock
Old report removed
New report generated
Old llapio_summary*.csv llap_mintimes_summary*.csv removed
Old logs removed
Log folder generated


^C
-bash-4.2$ cat query_runner.sh
#!/bin/bash
...
function generateZipReport() {
    # Final report location
    FINAL_REPORT_LOCATION="/tmp/PSE-Benchmarks_PVK/pkatti/TPC-DS/TPCDS_RESULTS/${SCALE}/"
    hdfs dfs -mkdir -p "${FINAL_REPORT_LOCATION}"
    hdfs dfs -chmod 777 "${FINAL_REPORT_LOCATION}"
